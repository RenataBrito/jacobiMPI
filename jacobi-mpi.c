...
*/
// to compile: mpicc jacobi-mpi.c -o jacobipar -fopenmp
// to run: mpirun -np 1 ./jacobipar N P T
...
int main(int argc, char *argv[])
{
    ...
    if(parentcomm == MPI_COMM_NULL) //Processo PAI Bootstrap do programa.
    {
        ...
        if(P<=N){ // Invocação dos processos filhos.
            MPI_Comm_spawn( "jacobipar", argv, P, MPI_INFO_NULL, root, MPI_COMM_WORLD, &intercomm, errcodes );
        }else{
            MPI_Comm_spawn( "jacobipar", argv, N, MPI_INFO_NULL, root, MPI_COMM_WORLD, &intercomm, errcodes );
        }
